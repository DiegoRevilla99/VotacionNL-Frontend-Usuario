...
export const ModalTokenVotacion = ({ statusModal, handleCloseModal, isOnModalToken }) => {
  ...
  const handleSubmit = (values) => {
    dispatch(
      onComenzarVotacion(values.token, username, () => navigate("/votacion/boletas"), jornadaFormal)
    );
  };
        ...
        <Box sx={{ overflowY: "auto", height: "100%" }}>
          ...
          <Box m={"2rem"}>
            <Typography variant="subtitle2" align="justify">
              El código de verificación es una pequeña secuencia de numeros que ha sido enviada como
              mensaje a tu teléfono. Para poder ingresar al módulo de votación, deberás copiar el
              código de verificación en el siguiente campo.
            </Typography>
              ...
              {({ values, handleSubmit, handleChange, errors, touched }) => (
                <form onSubmit={handleSubmit}>
                  ...
                  {errorMessage === "El código es incorrecto o ha caducado." ? (
                    <Box pt={2}>
                      <Alert severity="error">El código es incorrecto o ha caducado.</Alert>
                    </Box>
                  ) : (
                    <></>
                  )}

                  <Grid container pt={4} direction={{ xs: "column", md: "row-reverse" }}>
                    <Grid item xs={12} md={5} pb={{ xs: "2rem", md: "0" }}>
                      <Button
                        ...
                        startIcon={
                          status === "checking" ? <CircularProgress color="darkButton" /> : ""
                        }
                      >
                    ...
                    <Grid item xs={12} md={5}>
                      <Button
                        ...
                        color="error"
                        variant="outlined"
                        disabled={status === "checking" || isRunning ? true : false}
                        sx={{ flexGrow: "1" }}
